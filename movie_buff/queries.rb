...
def vanity_projects
  # List the title of all movies in which the director also appeared as
  #the starring actor. Show the movie id and title and director's name.

  # Note: Directors appear in the 'actors' table.
  Movie.joins(:actors).select(:id, :title, :name).where("movies.director_id = castings.actor_id AND castings.ord = 1 ")
end
...
def starring(whazzername)
  ...
  Movie.joins(:actors).select(:name).where('actors.name IN ?', whazzername)
...
def golden_age
  ...
  Movie.select(:yr).group(:yr).average(:score)
end
...
def costars(name)
  # List the names of the actors that the named actor
  # has ever appeared with.
  # Actor.joins(:actors).select(:name).where
end
